...
const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setSuccessMessage(null);
try {
  console.log("Trying login:", `${API_BASE_URL}/api/auth/login`);
...
const storedNotifications = JSON.parse(
  localStorage.getItem("NextHire_notifications") || "[]"
);

storedNotifications.unshift({
  id: Date.now(),
  title: "New Login",
  message: `${data.user.name} logged in successfully`,
  time: new Date().toLocaleString(),
  read: false,
});

localStorage.setItem(
  "NextHire_notifications",
...
setSuccessMessage("Logged in successfully! Redirecting...");
  ...
  <div className="relative">
    ...
    <input
      ...
      required
      disabled={isLoading}
      className="w-full bg-card border border-border rounded-xl pl-11 pr-12 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all disabled:opacity-50"
    />
...
<button
  type="submit"
  disabled={isLoading}
  className="w-full py-4 bg-gradient-to-r from-primary to-secondary text-white font-medium rounded-xl mt-4 flex items-center justify-center gap-2"
>
  {isLoading ? (
    <Loader2 className="w-5 h-5 animate-spin" />
  ) : (
